...
const Service = () => {
    ...
    return (
        <div className='h-100 lg:h-screen w-full flex justify-center items-center relative '>
            <img src={bg_img} alt="" />
            <div className="h-full w-full flex flex-col justify-center tablet:items-center gap-3 lg:gap-5 absolute mb-10 pl-5 lg:pl-0 ">
                <p className='text-[30px] tablet:text-[40px] text-[#0096e8] bold-futura font-bold '>Our Service</p>
                ...
                <div className="h-40 tablet:h-50 lg:h-65 container flex whitespace-nowrap overflow-auto gap-5 lg:pl-15 scroll-smooth ">
                    {items_array.map((data, index) => (
                        <div key={index} className="h-full w-40 tablet:w-50 lg:w-66 flex shrink-0 flex-col items-center text-center bg-[white] rounded-[20px] py-3 lg:py-5 gap-2 lg:gap-5  ">
                            <div className="h-15 w-14 tablet:h-17 tablet:w-17 lg:h-23 lg:w-23 rounded-full bg-[#0096e8] "></div>
                            <p className='max-w-[60%] text-[11px] whitespace-pre-wrap text-center tablet:text-[16px] lg:text-[18px] normal-futura '>{data.text} </p>
                            <Button lable={"Read More"} className={" bg-[#0096e8] text-[10px]! py-1.5! mt-1 tablet:text-[15px] lg:text-[16px] lg:px-9 lg:py-2! lg:scale-[1.25] "} />
                        </div>
                    ))}
                </div>
                {/* </div> */}
