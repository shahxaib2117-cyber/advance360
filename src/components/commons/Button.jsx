...
const Button = ({ lable, className, send = false, arrow = false, onclick, type = "button", isLoading, disabled }) => {

    return (
        <button type={type} onClick={onclick} disabled={disabled} className={`px-5 py-2 cursor-pointer rounded-md bg-[#1B1C67] text-[white] text-[10px] whitespace-nowrap md:text-[16px] flex items-center font-semibold ${className} `}>
